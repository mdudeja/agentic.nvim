...
This is a simple neovim plugin that allows you to interact with the the following AI tools:

- Copilot CLI (Needs to be installed and configured on your system)
- OpenCode CLI (Needs to be installed and configured on your system)
- Gemini CLI (Needs to be installed and configured on your system)
...
- Names of changed files in the TUI window with an ability to accept or reject the changes. This can be done via:
  - `a` -> accept all changes in the file
  - `d` -> reject all changes in the file
- Custom Snacks Window to View the changes in a diff view (using gitsigns) to see and accept or reject the changes. This can be done via:
  - `a` -> accept the change
  - `d` -> reject the change
